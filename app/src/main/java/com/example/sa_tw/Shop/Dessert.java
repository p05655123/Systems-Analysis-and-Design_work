...
public class Dessert extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        next.setOnClickListener(show_dessert_1_gui);
        order.setOnClickListener(intocart);
        cart.setOnClickListener(show_shoppingcart_gui);

        mediator.get_ditem1("2","chocolate cake");
    ...
    private void setTimerTask1() {
        mTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                mediator.output_dname1();
                mediator.output_dprice1();
                mediator.output_dinventory1();
                handler.sendEmptyMessage(0);
            }
        }, 500);
    }
    private View.OnClickListener show_dessert_1_gui = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            mediator.show_dessert_1_GUI();
        }
    };
        ...
        public void onClick(View v) {
            if(((Integer.valueOf(String.valueOf(q.getText())))<= 0) || (q.getText().toString().trim().isEmpty())){
                ...
                if(Integer.valueOf(q.getText().toString()) > inventory){
                    ...
                    mediator.intocart(3,Integer.valueOf(q.getText().toString()),Integer.valueOf(q.getText().toString())*price);
                    q.setText("");
                }
            }
        }
